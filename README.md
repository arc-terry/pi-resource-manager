...
`pi-collection` catalogs a reviewed set of Pi packages, skills, and extensions. The CLI calls Pi for package installation and removal; it never runs `npm install` or clones Git repositories itself. In command output and options, Pi extensions are called **plugins**.
...
The package exposes the `pi-collection` binary after `npm run build` when installed through npm.
...
pi-collection install [names...] [--full] [--type package|skill|plugin] [--yes]
pi-collection remove <names...> [--type package|skill|plugin] [--yes]
pi-collection add <source> [--local] [--type package|skill|plugin] [--dry-run] [--yes]
```
...
```sh
pi-collection list
pi-collection list --type plugin --full
pi-collection install review-workflow --yes
pi-collection install --full --yes
pi-collection remove review-workflow --yes
pi-collection add npm:@acme/pi-tools@1.2.3 --type plugin --yes
pi-collection add ./my-local-pi-package --local --dry-run
```
...
pi-collection profile restore <profile.yml> [--dry-run] [--only package|skill|plugin] [--project <path>] [--yes]
```
